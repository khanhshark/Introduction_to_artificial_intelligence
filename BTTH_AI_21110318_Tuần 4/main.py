        ...
        for polygon in self.polygons:
            for edge in self.polygons[polygon]:
                for point in self.get_points():
                 ...
                 #   Kiểm tra điểm xuất phát start:
                    if start == point:
                        cant_see_list.append(point)
                # Kiểm tra nếu điểm start có trong các điểm của đa giác:
                    if start in self.get_polygon_points(polygon):
                        for poly_point in self.get_polygon_points(polygon):
                            # nếu không phải là điểm kề thì thêm vào tập không nhìn thấy
...
def search(graph, start, goal, func):
    ...
    while not queue.empty():
        ...
        for i in graph.can_see(node):
            ...
            # i chưa có hoặc i đã có mà chi phí củ lơn hơn chi phí mới thì cập nhật lại
